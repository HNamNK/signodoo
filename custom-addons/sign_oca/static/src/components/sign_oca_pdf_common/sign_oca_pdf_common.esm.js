...
// 🔥 QUAN TRỌNG: Import SignRegistry
...
export default class SignOcaPdfCommon extends Component {
    ...
    postIframeField(item) {
        console.log(`[DEBUG] Processing item:`, item.field_type, item.name);
        
        if (this.items[item.id]) {
            this.items[item.id].remove();
        }
        
        var page = this.iframe.el.contentDocument.getElementsByClassName("page")[item.page - 1];
        var signatureItem = $(
        ...
        this._renderFieldContent(item, signatureItem);
    ...
    _renderFieldContent(item, signatureItem) {
        ...
        const isConfigureMode = this.field_template === "sign_oca.sign_iframe_field_configure";
        if (item.field_type === 'auto_fill' && isConfigureMode) {
            return; // Template đã hiển thị name, không cần thêm gì
        }
        
        try {
            const fieldComponent = SignRegistry.get(item.field_type);
            
            if (fieldComponent && fieldComponent.generate) {
                const fieldElement = fieldComponent.generate(this, item, signatureItem);
                
                if (fieldElement) {
                    signatureItem[0].append(fieldElement);
        ...
        if (!isConfigureMode) {
            this._renderFallbackContent(item, signatureItem);
        } else {
        }
    }
    
    _renderFallbackContent(item, signatureItem) {
        const fallbackElement = document.createElement('div');
        
        // Auto-fill: hiển thị sẵn value (hoặc default_value), nhưng style vẫn dùng chung
        let displayText;
        if (item.field_type === 'auto_fill') {
            displayText = item.value || item.default_value 
        ...
        fallbackElement.textContent = displayText;
        fallbackElement.classList.add("o_sign_oca_field");   // <-- dùng class chung
